# Programme probabiliste FDJ.py - VERSION AVEC DONNÉES DE TEST
...
def generer_donnees_test(nb_tirages: int = 500) -> List[Dict[str, Any]]:
    """Génère des données de test réalistes pour le Loto français"""
    logging.info(f"Génération de {nb_tirages} tirages de test...")
    
    data = []
    # Probabilités légèrement biaisées pour simuler des numéros plus/moins fréquents
    probas_main = [1.0] * 49
    # Quelques numéros plus fréquents
    for num in [7, 13, 21, 32, 45]:
        probas_main[num-1] = 2.0
    # Quelques numéros moins fréquents  
    for num in [4, 18, 26, 39, 48]:
        probas_main[num-1] = 0.7
    
    # Probabilités pour le numéro chance
    probas_chance = [1.0] * 10
    probas_chance[2-1] = 1.5  # Le 2 est plus fréquent
    probas_chance[7-1] = 1.3  # Le 7 est un peu plus fréquent
    
    for i in range(nb_tirages):
        # Tirage des 5 numéros principaux sans remise
        numeros_main = []
        numeros_possibles = list(range(1, 50))
        probas_temp = probas_main.copy()
        
        for _ in range(5):
            if not numeros_possibles:
                break
            # Normaliser les probabilités
            probas_norm = [probas_temp[num-1] for num in numeros_possibles]
            probas_norm = [p/sum(probas_norm) for p in probas_norm]
            
            numero_tire = random.choices(numeros_possibles, weights=probas_norm, k=1)[0]
            numeros_main.append(numero_tire)
            
            # Retirer le numéro tiré et sa probabilité
            idx = numeros_possibles.index(numero_tire)
            numeros_possibles.pop(idx)
            probas_temp.pop(idx)
        
        # Tirage du numéro chance
        numero_chance = random.choices(range(1, 11), weights=probas_chance, k=1)[0]
        
        data.append({
            "main": sorted(numeros_main),
            "chance": numero_chance
        })
    
    logging.info(f"Génération terminée : {len(data)} tirages créés")
    return data

def compresser_donnees_base64(data: List[Dict[str, Any]]) -> str:
    """Compresse les données en Base64 pour simuler le format original"""
    json_str = json.dumps(data)
    bytes_io = io.BytesIO()
    with gzip.GzipFile(fileobj=bytes_io, mode='w') as f:
...
def charger_donnees_test() -> List[Dict[str, Any]]:
    """Charge les données de test générées"""
    return generer_donnees_test(300)
...
def main() -> None:
    # Chargement des données de test générées
    historique_loto = charger_donnees_test()
    ...
    parser.add_argument("--analyse", type=str, default="stats",
                        help=("Type d'analyse à exécuter: stats, simulation, montecarlo, predict, "
    ...
    parser.add_argument("--predictor", type=str, default="logistic",
                        help=("Modèle prédictif à utiliser (si --analyse predict) : "
                              "logistic, ensemble, stacking, deep, lstm"))
